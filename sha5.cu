 ...
 */
 __device__  static void rhash_sha512_process_block(uint64_t hash[8], uint64_t block[16])
{
 ...
 */
 __device__  void rhash_sha512_update(sha512_ctx *ctx, const unsigned char *msg, size_t size)
{
...
__device__ void SHA512(void *digest,const void *data,unsigned int len)
{
    ...
    rhash_sha512_update (&ctx, (unsigned char *)data, len);
    rhash_sha512_final (&ctx, (unsigned char *)digest);
}